...
class Resize:
    """Resize images & keypoints & mask.
    This transform resizes the input image to some scale. Keypoints and masks are
    ...
    def _resize_joints(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        results['joints'] = results['joints'] * results['scale_factor']

    def _resize_masks(self, results):
        """Resize masks with ``results['scale']``"""
        for i, mask in enumerate(results['mask']):
            if self.keep_ratio:
                results['mask'][i] = results['mask'][i].rescale(results['scale'])
            else:
                results['mask'][i] = results['mask'][i].resize(results['img_shape'][:2])
